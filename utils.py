...
def compute_warped_images(folder, neighbor_box_distance=50, facial_rec_algo='ssd', ocr_method='EasyOCR', rotation_interval='30'):
    res = []
    ...
    face_detector = detect_face.face_factory(face_model = facial_rec_algo)
    ...
    for filename in sorted(os.listdir(folder)):
        ...
        res.append((filename, final_img))
    
    
    os.system('mkdir -p ./unwarped/')
    for fn, im in res:
        cv2.imwrite(fn, im)
